...
class Database:
    ...
    async def connect(self):
        ...
        try:
            ...
            print(f"Database: {os.getenv('POSTGRES_DB', 'voiceai')}")
            print(f"User: {os.getenv('POSTGRES_USER', 'voiceai')}")
            
            self.pool = await asyncpg.create_pool(
                user=os.getenv("POSTGRES_USER", "voiceai"),
                password=os.getenv("POSTGRES_PASSWORD", "Firstlaunch4321"),
    ...
    async def get_call_session(self, call_id: str):
        """通話セッション情報を取得"""
    ...
    async def save_message(self, call_id: str, content: str, type: str):
        """メッセージをデータベースに保存"""
        async with self.pool.acquire() as conn:
            await conn.execute(
                """
                INSERT INTO messages (call_id, content, type, created_at)
                VALUES ($1, $2, $3, $4)
                """,
                call_id, content, type, datetime.utcnow()
            )
        ...
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow(
                ...
                INSERT INTO fax_documents (
                    tenant_id, direction, sender_number, receiver_number,
                    status, tiff_path, pdf_path, ocr_text, processed_at
                )
                VALUES ($1, $2, $3, $4, $5, $6, $7, $8, now())
                RETURNING *
                """,
                tenant_id, direction, sender_number, receiver_number,
                status, tiff_path, pdf_path, ocr_text
            )
            return FaxDocument(**dict(row))
        ...
        async with self.pool.acquire() as conn:
            return await conn.fetchrow(
                ...
                INSERT INTO call_sessions (
                    id, tenant_id, from_number, to_number, 
                    start_time, status
                )
                VALUES ($1, $2, $3, $4, now(), $5)
                RETURNING *
                """,
                call_id, tenant_id, from_number, to_number, status
            )
    ...
    async def get_call_sessions_count(self, tenant_id: str) -> int:
        """通話セッション数を取得"""
        async with self.pool.acquire() as conn:
    ...
    async def save_dtmf(
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                """
                INSERT INTO dtmf_events (call_id, digit, created_at)
                VALUES ($1, $2, $3)
                """,
                call_id, digit, timestamp or datetime.utcnow()
            )
    
    async def get_tenant_greeting(self, tenant_id: str) -> Optional[dict]:
        """テナントの挨拶メッセージ設定を取得"""
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow(
                """
    ...
    async def get_tenant_settings(self, tenant_id: str) -> Optional[dict]:
        """テナントの全設定を取得"""
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow(
    ...
    async def create_tenant_settings(self, tenant_id: str, settings: dict) -> dict:
        """テナント設定を作成"""
        async with self.pool.acquire() as conn:
            # カラム名と値を動的に構築
            columns = ['tenant_id'] + list(settings.keys())
            placeholders = [f'${i+1}' for i in range(len(columns))]
            values = [tenant_id] + list(settings.values())
            
            query = f"""
                INSERT INTO tenant_settings ({', '.join(columns)})
    ...
    async def update_tenant_settings(self, tenant_id: str, settings: dict):
        """テナント設定を更新"""
        async with self.pool.acquire() as conn:
            if not settings:
                return
            
            set_clauses = [f"{key} = ${i+2}" for i, key in enumerate(settings.keys())]
            query = f"""
                UPDATE tenant_settings
                SET {', '.join(set_clauses)}, updated_at = NOW()
                WHERE tenant_id = $1
            """
            await conn.execute(query, tenant_id, *settings.values())
    ...
    async def get_active_calls(self, tenant_id: str) -> List[dict]:
        """アクティブな通話一覧を取得"""
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                """
